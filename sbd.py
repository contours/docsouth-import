...
## globals
#SVM_LEARN = '/usr/bin/svm_learn'
#SVM_CLASSIFY = '/usr/bin/svm_classify'
SVM_LEARN = '~/linux-x86_64/bin/svm_learn'
SVM_CLASSIFY = '~/linux-x86_64/bin/svm_classify'
...
class SVM_Model(Model):
    ...
    def train(self, doc):
        ...
        fd, train_file = tempfile.mkstemp()
        fh = os.fdopen(fd, 'w')
        fh.write('\n'.join(lines) + '\n')
        fh.close()
    ...
    def classify(self, doc, verbose=False):
        ...
        ## get predictions
        total = 0
        preds = map(float, os.fdopen(fd).read().splitlines())
        frag = doc.frag
        while frag:
